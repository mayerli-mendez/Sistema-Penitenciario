...
export const DirectorForm = ({ director }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (director?.id) {
                await axios.post(
                    `https://apiterminado.herokuapp.com/api/v1/director/${director.id}/update`,
                    { ...form }, { headers: { 'accept': 'application/json', 'authorization': token } }
                );
            } else {
                await axios.post(
                    `https://apiterminado.herokuapp.com/api/v1/director/create`,
                    { ...form }, { headers: { 'accept': 'application/json', 'authorization': token } }
                );
            }
            navigate('/directors');
            ...
            <div className="panel-body">
                <form onSubmit={handleSubmit}>
                    ...
                    <p className="text-center">
                        <button value={director?.id ? 'Actualizar' : 'Guardar'} type="submit" className="btn btn-info btn-raised btn-sm"><i className="zmdi zmdi-floppy"></i> GUARDAR</button>
                    </p>
                </form>
            </div>
