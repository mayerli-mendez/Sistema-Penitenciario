...
export const PrisonerForm = ({ prisoner }) => {
    ...
    const [form, setForm] = useState({
        ...
        personal_phone: prisoner?.personal_phone ?? "",
        home_phone: prisoner?.home_phone ?? "",
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (prisoner?.id) {
                await axios.post(
                    `https://apiterminado.herokuapp.com/api/v1/prisoner/${prisoner.id}/update`,
                    { ...form }, { headers: { 'accept': 'application/json', 'authorization': token } }
                );
            } else {
                await axios.post(
                    `https://apiterminado.herokuapp.com/api/v1/prisoner/create`,
                    { ...form }, { headers: { 'accept': 'application/json', 'authorization': token } }
                );
            }
            navigate('/prisoners');
            ...
            <div className="panel-body">
                <form onSubmit={handleSubmit}>
                        ...
                        <div className="container-fluid">
                            <div className="row">
                                ...
                                <div className="col-xs-12">
                                    <div className="form-group label-floating">
                                        <label htmlFor='address' className="control-label">Reclusos</label>
                                        <textarea
                                            className="form-control"
                    ...
                    <p className="text-center">
                        <button value={prisoner?.id ? 'Actualizar' : 'Guardar'} type="submit" className="btn btn-info btn-raised btn-sm"><i className="zmdi zmdi-floppy"></i> GUARDAR</button>
                    </p>
                </form>
            </div>
